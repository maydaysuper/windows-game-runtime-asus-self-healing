...
function Get-BuildConsistency {
    ...
    try{$a=(Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $PSScriptRoot 'RepairCenter.ps1')).Hash.ToLowerInvariant();AddC 'Engine SHA256' $(if($a -eq [string]$BuildInfo.EngineSHA256){'PASS'}else{'FAIL'}) ([string]$BuildInfo.EngineSHA256) $a ''}catch{AddC 'Engine SHA256' 'FAIL' ([string]$BuildInfo.EngineSHA256) '' $_.Exception.Message}
