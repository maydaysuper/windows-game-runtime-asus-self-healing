...
Uninstall every 3.4.x build first. Setup installs per-user under LocalAppData and puts **自愈中心** on the desktop. It does not add a Start Menu shortcut. Double-click the desktop icon, or unzip the portable folder and double-click `SelfHealingCenter.exe`. UAC appears only when an elevated ASUS / GPU repair actually runs.
