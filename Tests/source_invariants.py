...
if installer.exists():
    ...
    if all(x in raw for x in ['PrivilegesRequired=lowest','MinVersion=10.0.22000','ArchitecturesAllowed=x64compatible','recursesubdirs','WindowsGameRuntimeASUSSelfHealing.WinUI.exe']): ok('Setup installer packages complete Win11 x64 runtime per-user')
...
if all(x in workflow for x in ['Verify published payload trust chain','JRSoftware.InnoSetup.7','Build-Release.ps1','artifacts/release/*','-p:Platform=$env:PLATFORM','softprops/action-gh-release','Show-MsBuildErrors','contents: write']): ok('Windows CI emits verified installer + portable release artifacts')
