...
if '<WindowsAppSdkBootstrapInitialize>false</WindowsAppSdkBootstrapInitialize>' in csproj and '<WindowsAppSdkDeploymentManagerInitialize>false</WindowsAppSdkDeploymentManagerInitialize>' in csproj: ok('self-contained unpackaged disables WASDK bootstrap FailFast')
else: fail('WindowsAppSdkBootstrapInitialize must be false for self-contained; v3.4.10 Fail Fast Exception')
...
if 'Background="{ThemeResource SystemAccentColorLight2}"' in main_xaml: fail('MainWindow must not use Color theme resource as Brush')
else: ok('MainWindow accent color is wrapped as SolidColorBrush')
...
if "-p:PublishSingleFile=false" in one and 'Microsoft.ui.xaml.dll' in one and 'WindowsAppSdkBootstrapInitialize=false' in one: ok('OneClick forbids WinUI PublishSingleFile payload')
